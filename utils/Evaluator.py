...
class Evaluator():
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        # 我这边添加了data_range 可能会有问题
        #data_range1 = max(image_F.max(), image_A.max()) - min(image_F.min(), image_A.min())
        #data_range2 = max(image_F.max(), image_B.max()) - min(image_F.min(), image_B.min())
